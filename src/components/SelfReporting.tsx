...
export function SelfReporting() {
  ...
  const sendReport = useCallback(() => {
    ...
    // In production, send to your support endpoint or email service
    console.log("[BUG REPORT]", { message, diagnostics });
    
    // Store locally for admin review (until you wire up a backend endpoint)
    try {
        ...
        <div className="fixed bottom-12 left-2 z-[90] bg-[#1a2332] border border-white/10 rounded-xl p-4 shadow-2xl w-80">
          ...
          {sent ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-green-400 font-medium">Report sent!</p>
              <p className="text-xs text-gray-500 mt-1">We will review it shortly.</p>
            </div>
          ) : (
            <>
              ...
              <div className="bg-white/5 rounded-lg p-2.5 mb-3 space-y-1 text-[10px] text-gray-400">
                ...
                <div className="flex items-center gap-1.5">
                  <Smartphone size={10} /> {navigator.userAgent.includes("Mobile") ? "Mobile" : "Desktop"}
                </div>
              </div>
              ...
              >
                <Send size={12} className="mr-1.5" /> Send Report
              </Button>
            </>
          )}
        </div>
