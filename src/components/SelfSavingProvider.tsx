...
export function SelfSavingProvider({ children }: { children: React.ReactNode }) {
  ...
  const flush = useCallback(() => {
    ...
    queue.current.forEach((data, key) => {
      try {
        ...
        // Storage full — clear oldest autosaves
  ...
  const scheduleFlush = useCallback(() => {
    ...
    timer.current = setTimeout(flush, 2000); // batch saves every 2 seconds
  }, [flush]);
  ...
  // Periodic heartbeat save
  useEffect(() => {
    const interval = setInterval(flush, 30000); // flush every 30s regardless
