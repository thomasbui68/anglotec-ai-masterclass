...
export default function ProgressPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsData, achs] = await Promise.all([
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f8f9fa] to-white">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                <BookOpen className="text-[#1a365d]" size={24} />
                <div>
                  <p className="text-2xl font-bold text-[#1a365d]">{mastered}</p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-[#1a365d]">{learning}</p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-[#1a365d]">{stats?.total_practices || 0}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span>Mastered</span>
                  <span className="font-semibold">{mastered} ({Math.round((mastered / total) * 100)}%)</span>
                </div>
                <Progress value={(mastered / total) * 100} className="h-2 bg-gray-100" />
              </div>
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span>Learning</span>
                  <span className="font-semibold">{learning} ({Math.round((learning / total) * 100)}%)</span>
                </div>
                <Progress value={(learning / total) * 100} className="h-2 bg-gray-100" />
              </div>
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span>New</span>
                  <span className="font-semibold">{newCount} ({Math.round((newCount / total) * 100)}%)</span>
                </div>
                <Progress value={(newCount / total) * 100} className="h-2 bg-gray-100" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Mastery Roadmap</CardTitle>
            </CardHeader>
              ...
              {masteryStages.map((stage) => (
                <div
                  ...
                  className={`flex items-center gap-3 p-2 rounded-lg ${
                    currentStage.label === stage.label ? "bg-[#d4af37]/10 border border-[#d4af37]" : "bg-gray-50"
                  }`}
                >
                  <div className={`w-4 h-4 rounded-full ${stage.color}`} />
                  <div className="flex-1">
                    <p className={`font-semibold text-sm ${currentStage.label === stage.label ? "text-[#d4af37]" : "text-gray-700"}`}>
                      {stage.label}
                    </p>
            ...
            {achievements.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {achievements.map((ach) => (
                  <div
                    key={ach.id}
                    className="flex items-center gap-3 p-3 bg-gradient-to-r from-[#d4af37]/10 to-transparent rounded-lg border border-[#d4af37]/20"
                  >
